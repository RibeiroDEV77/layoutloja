...
function unwrap<T = any>(r: { ok: true; data: T } | { ok: false; error: { message: string } }): T {
...
function CustomerDetailPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => unwrap(await get360({ data: { customer_id: customerId } })),
  });
  ...
  const d = data as Any;
  const c = d.customer as Record<string, Any> | null;
    ...
    <div className="p-6 space-y-6 max-w-6xl">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge variant="outline">Crédito: R$ {Number(d.credit_balance ?? 0).toFixed(2)}</Badge>
          {(d.tags ?? []).filter(Boolean).map((t: Any) => (
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview"><OverviewTab data={d} onRefresh={refetch} /></TabsContent>
        <TabsContent value="timeline"><TimelineTab customerId={customerId} /></TabsContent>
        <TabsContent value="notes"><NotesTab customerId={customerId} notes={d.notes ?? []} onChange={refetch} /></TabsContent>
        <TabsContent value="tags"><TagsTab customerId={customerId} storeId={d.store_id} current={(d.tags ?? []).filter(Boolean)} onChange={refetch} /></TabsContent>
        <TabsContent value="consents"><ConsentsTab customerId={customerId} customer={c} consents={d.consents ?? []} onChange={refetch} /></TabsContent>
        <TabsContent value="addresses"><AddressesTab addresses={d.addresses ?? []} /></TabsContent>
      </Tabs>
    </div>
  );
}
        ...
        <CardContent className="space-y-1 text-sm">
          ...
          {(data.factors ?? []).map((f: Any) => (
            <div key={f.id} className="flex justify-between border-b py-1">
              ...
              <span className={Number(f.value) < 0 ? 'text-destructive' : ''}>{Number(f.value) > 0 ? '+' : ''}{Number(f.value)}</span>
            </div>
          ))}
...
function TimelineTab({ customerId }: { customerId: string }) {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => unwrap(await fn({ data: { customer_id: customerId, limit: 50 } })),
  });
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {rows.length === 0 && <div className="text-muted-foreground text-sm">Sem eventos.</div>}
      </CardContent>
    </Card>
  );
}

function NotesTab({ customerId, notes, onChange }: { customerId: string; notes: Any[]; onChange: () => void }) {
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" onClick={async () => {
              if (!body.trim()) return;
              try { unwrap(await add({ data: { customer_id: customerId, body, pinned } })); setBody(''); setPinned(false); refresh(); toast.success('Nota adicionada'); }
            ...
            <div key={String(n.id)} className="border rounded p-3 text-sm">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex gap-1">
                  <Button size="sm" variant="ghost" onClick={async () => { unwrap(await upd({ data: { id: String(n.id), pinned: !n.pinned } })); refresh(); }}>
                    <Pin className={`h-4 w-4 ${n.pinned ? 'fill-current' : ''}`} />
                  </Button>
                  <Button size="sm" variant="ghost" onClick={async () => { if (confirm('Excluir nota?')) { unwrap(await del({ data: { id: String(n.id) } })); refresh(); } }}>
...
function TagsTab({ customerId, storeId, current, onChange }: { customerId: string; storeId: string; current: Any[]; onChange: () => void }) {
  ...
  const { data: catalog, refetch } = useQuery({
    ...
    queryFn: async () => unwrap(await listFn({ data: { store_id: storeId } })),
  });
  ...
  const toggle = async (id: string) => {
    ...
    unwrap(await assignFn({ data: { customer_id: customerId, tag_ids: Array.from(next) } }));
    onChange();
  };
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex gap-2">
          ...
          <Button onClick={async () => {
            ...
            try { unwrap(await upsertFn({ data: { store_id: storeId, name: newTag } })); setNewTag(''); refetch(); toast.success('Tag criada'); }
            catch (e) { toast.error((e as Error).message); }
          }}>Criar</Button>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {list.length === 0 && <span className="text-sm text-muted-foreground">Nenhuma tag no catálogo.</span>}
        </div>
      </CardContent>
    </Card>
  );
...
function ConsentsTab({ customerId, customer, consents, onChange }:
  { customerId: string; customer: Record<string, Any>; consents: Any[]; onChange: () => void }) {
  ...
  const update = async (ch: typeof channels[number], granted: boolean) => {
    try { unwrap(await fn({ data: { customer_id: customerId, consents: { [ch]: granted } } })); onChange(); toast.success('Consentimento atualizado'); }
    catch (e) { toast.error((e as Error).message); }
  };
  return (
    <div className="grid gap-4 md:grid-cols-2">
